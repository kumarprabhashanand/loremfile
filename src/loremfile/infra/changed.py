...
#: When `main`'s history was rewritten, or None if it never has been. Unset has no effect.
#:
#: A rewrite replaces every commit SHA, so a run created before it names a commit this
#: checkout no longer has. `changed_paths` rightly calls that `Undecided` — which, left
#: alone, would fail every push deploy from the rewrite on. So a run created before the
#: cutoff is never a base: the first push deploy after a rewrite finds none and counts
#: `infra/` as changed, and `apply` compares before it writes, so that costs reads, not
#: writes. A base created *after* the cutoff that the checkout lacks is still `Undecided`.
HISTORY_REWRITTEN_AT: dt.datetime | None = dt.datetime(2026, 9, 21, 12, 3, 25, tzinfo=dt.UTC)
...
def _created(run: Mapping[str, Any]) -> dt.datetime:
    """A run's `created_at`, timezone-aware. Read only when a rewrite cutoff is set."""
    value = run.get("created_at")
    if not isinstance(value, str):
        raise Undecided(f"workflow run {run['id']} has no usable 'created_at': {value!r}")
    try:
        created = dt.datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise Undecided(
            f"workflow run {run['id']} has an unreadable 'created_at': {value!r}"
        ) from exc
    if created.tzinfo is None:
        raise Undecided(f"workflow run {run['id']} has a 'created_at' with no timezone")
    return created


def select_base(
    runs: Iterable[object], *, exclude_run: int, rewritten_at: dt.datetime | None = None
) -> Base | None:
    """The run the zone last converged to, or None when no run qualifies.

    Qualifies: a push to main that completed with `success`, and is not `exclude_run` — and,
    when `rewritten_at` is set, was created at or after it (see `HISTORY_REWRITTEN_AT`). The
    most recent is the highest `run_number` — not whichever the API happened to list first.
    """
    if rewritten_at is not None and rewritten_at.tzinfo is None:
        raise Undecided("the history-rewrite cutoff has no timezone, so no run can be compared")
    qualifying = [
        run
    ...
    if rewritten_at is not None:
        qualifying = [run for run in qualifying if _created(run) >= rewritten_at]
    if not qualifying:
        return None
...
def decide(
    *, repository: str, exclude_run: int, head: str = "HEAD", cwd: Path | None = None
    ...
    base = select_base(
        fetch_runs(repository), exclude_run=exclude_run, rewritten_at=HISTORY_REWRITTEN_AT
    )
    if base is None:
        since = (
            f" since {BRANCH}'s history was rewritten at {HISTORY_REWRITTEN_AT.isoformat()}"
            if HISTORY_REWRITTEN_AT is not None
            else ""
        )
        return Decision(
            changed=True,
            reason=f"no earlier successful push run of {WORKFLOW} on {BRANCH}{since}; "
