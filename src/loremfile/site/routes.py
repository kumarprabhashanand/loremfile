...
#: Files that exist to be *shown* — a tab icon, a home-screen icon, the social card and the
#: mark — so search engines and social platforms must be able to index them. They are the
#: only dotted paths the `files_noindex` header rule excludes (`08` §5.3); a test pins the
#: rule's paths to this set so the two cannot drift apart.
DISPLAY_ASSETS: Final = frozenset(
    {"favicon.ico", "apple-touch-icon.png", "assets/og.png", "assets/mark.svg"}
)
CONTENT_TYPES: Final = {
