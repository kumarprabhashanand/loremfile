...
class LegalReport:
    ...
    def problems(self) -> list[str]:
        checks = (
            ...
            (
                self.forbidden_in_git,
                "string(s) from FORBIDDEN_STRINGS are in the repository: remove them; the "
                "list itself lives only in that secret",
            ),
        )
    ...
    def render(self) -> str:
        return (
            ...
            f"{self.placeholders_left}; values in git {self.values_in_git}; forbidden "
            f"strings {self.forbidden_checked} checked, {self.forbidden_in_git} in git"
        )


def forbidden_from(env: Mapping[str, str]) -> list[str]:
    """`FORBIDDEN_STRINGS`, one string per line — optional, and never logged.

    The denylist lives only in the production secret: committed to the repository it would
    publish exactly what it exists to keep out. Blank lines are dropped because `git grep -F`
    with an empty pattern matches every line of every file, so one stray newline in the secret
    would fail every build and teach everyone to ignore the guard.
    """
    raw = env.get("FORBIDDEN_STRINGS", "")
...
def audit(
    ...
    *,
    in_git: Callable[[str], bool],
    forbidden: Sequence[str] = (),
    forbidden_in_git: Callable[[str], bool] | None = None,
) -> LegalReport:
    ...
    report.forbidden_checked = len(forbidden)
    check = forbidden_in_git or in_git
    report.forbidden_in_git = sum(check(text) for text in forbidden)
    return report


def in_repository(value: str, *, cwd: Path, ignore_case: bool = False) -> bool:
    """`git grep -qF` for one value; the value is an argument, never output (docs/13 §3b).

    `ignore_case` is for the `FORBIDDEN_STRINGS` denylist: a name is the same name in any
    capitalisation, and a case-sensitive check would pass on the first capitalised mention.
    The IMPRINT values keep the exact match they were verified with."""
    executable = shutil.which("git")
    if executable is None:
        raise LegalError("git is not on PATH")
    done = subprocess.run(  # noqa: S603 - fixed argv, no shell
        [executable, "grep", "-qF", *(["-i"] if ignore_case else []), "-e", value],
        cwd=cwd,
        capture_output=True,
        check=False,
    )
