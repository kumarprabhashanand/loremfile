...
| **pyzipper** | AES salt/IV come from the patched `os.urandom` | `zip/aes256-password-loremfile.zip` — the only AES archive, so the only salt/IV | **unverified — spike in M3.7** |
| **py7zr** | Timestamps come from the patched clock | `7z/3-text-files.7z` — 7z stores a timestamp per entry | **unverified — spike in M3.7** |
