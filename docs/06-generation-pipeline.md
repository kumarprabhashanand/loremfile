...
fixtures:
  - name: a4-3pages.pdf
    ...
    edge: null                  # for fixtures under edge/ → { intended_format: "pdf", defect: "truncated", compare_with: "pdf/a4-3pages.pdf", outcome: "may-recover", source_fixture: "pdf/a4-3pages.pdf", fraction: 0.6 }  (fraction only for truncated; magic only for magic-prefix)
```

Field rules: `name`, `phase`, `generator`, `description`, `tags`, `size_class` are required; `expect` is required unless `edge_case: true`; `nominal_bytes` is required for `exact`/`boundary`/`approx`; `edge` is required for every fixture under `edge/` (its `defect` must be one of the closed enum in `04-manifest-and-discovery.md` §1.3, and `truncated` takes a `fraction`, `magic-prefix` a `magic` value; `compare_with` names the valid fixture of the same announced type and `outcome` is one of `must-fail`, `may-recover`, `varies`, which the validator derives from the bytes and refuses when the two disagree — `04` §1.3.1); fixtures outside `edge/` may set `edge_case: true` without an `edge` block (they are valid files that are merely unusual, e.g. `txt/control-characters.txt`); `removed` is required when `status: removed`. Everything else is optional with the defaults shown. Generators declare `parallel_safe` themselves (see §4); it is not a catalog field.
...
**The proving-fixture column says why that fixture is probative, not just which one it is.** The cross-check test can prove a fixture is parametrised; it cannot prove the fixture exercises the claim. M3.6 found the mutagen row naming `mp3/sine-440hz-3s.mp3`, which carries no tags — a wrong row that ran a passing test. Stating the reason moves that error from "requires running the spike" to "visible while reading".

| Library | Claim | Proving fixture | Verified |
|---|---|---|---|
| fastavro | Sync marker is **not** reachable by the guard — `fastavro._write` is a compiled C extension — so it is passed explicitly as `sync_marker=ctx.stream(16)` | `avro/people-1000.avro` — every block is delimited by the sync marker | 2026-09-07 (claim corrected) |
| avifenc | Invoked `--jobs 1 --speed 6`; multi-threaded AV1 makes timing-dependent choices | `avif/640x480.avif` — the only fixture avifenc encodes | 2026-09-07 |
| fpdf2 | Encryption IV comes from the patched `os.urandom`; creation date, producer and creator set explicitly because fpdf2 formats the date itself | `pdf/a4-encrypted-1page.pdf` — the only encrypted PDF, so the only one with an IV | 2026-09-08 |
| pyarrow | Parquet and Arrow are deterministic for a pinned version; the pandas metadata block is suppressed because it embeds a pandas version string | `parquet/people-1000.parquet` — pyarrow writes it, metadata block included | 2026-09-07 |
| Pillow | `ImageFont.load_default(size)` and every save path are deterministic for a pinned Pillow | `png/640x480.png` — rendered and saved entirely by Pillow | 2026-09-07 |
| SQLite | Fixed `page_size`, `journal_mode=DELETE` and a closing `VACUUM` make the page layout depend on contents, not insertion order | `sqlite/people-1000.sqlite` — multi-page, so page layout can vary | 2026-09-07 |
| `util.zipnorm` | Sorted entries, fixed timestamps and permissions make any zip-based file byte-stable | `kmz/placemarks-10.kmz` — a zip whose entries zipnorm restamps | 2026-09-07 |
| **pyzipper** | **Claim corrected in M3.7.** The AES salt comes from `Cryptodome.Random.new().read()`, which reads OS randomness in C and never reaches the patched `os.urandom`: three builds gave three different archives. `util.determinism` now patches `Cryptodome.Random` as well, and the archive is byte-stable | `zip/aes256-password-loremfile.zip` — the only AES archive, so the only salt/IV | 2026-09-16 (claim corrected) |
| **py7zr** | Entry timestamps come from the staged files, whose mtime the generator sets to the project epoch — py7zr archives files from disk, so the patched clock never applies to them | `7z/3-text-files.7z` — 7z stores a timestamp per entry | 2026-09-16 |
| **cryptography** | X.509 serial fixed to `0x4c6f72656d66696c65`, subject/issuer `CN=fixture.example, O=loremfile fixtures`, SAN `fixture.example` (never the production hostname), validity 2020-01-01 to 2120-01-01, Ed25519 key from `sha256(b"loremfile:cert:ed25519")` and never written to disk | `pem/self-signed-ed25519-cert.pem` — the only certificate, so the only serial and key | **unverified — spike in M3.7** |
| mutagen | ID3v2.3 frames written with fixed values and no timestamps of any kind — `TDRC` is the catalog's literal year, not the build date, and no `TDEN`/`TDTG` is emitted. Cover art is a published PNG read through `ctx.dependency` | `mp3/with-id3v2-tags-3s.mp3` — **the only fixture carrying ID3 frames at all** | 2026-09-09 (proving fixture corrected: the row named `sine-440hz-3s.mp3`, which carries no tags at all) |
| ffmpeg | `-fflags +bitexact -flags:v +bitexact -flags:a +bitexact -map_metadata -1` and an emptied encoder tag stop the version and build stamps reaching the container; `-threads 1` (and `-x264-params threads=1`, `-row-mt 0`) because multi-threaded x264 and VP9 both make timing-dependent slice decisions. Verified across all 14 encoder recipes M3.6 uses, in separate processes **and** in invocations minutes apart | `mp4/360p-5s.mp4` — video and audio through the full ffmpeg pipeline | 2026-09-09 |
