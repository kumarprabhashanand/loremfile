...
**Implemented M4.4, without the site steps.** `loremfile site build` and the site-key half of `verify-live` arrive with M4.1; a step that hashed an empty `build/site/` would report a clean **defacement** check for a site that does not exist, which is the worst possible shape for that particular check. The cost and rotation thresholds are applied in the workflow rather than inside the commands, because a threshold belongs to the check and `usage`/`tokens-due` should stay plain readers.
