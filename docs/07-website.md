...
- **Trailing slashes**: canonical URLs have none. The edge rewrites a trailing slash to the extensionless key, so `/{format}/`, `/docs/` and `/docs/faq/` all work (ADR-032).
- **Legal links in the footer of every page, one click away, labelled exactly **"Impressum"** and **"Privacy"**** (owner decision 2026-09-21; until then they were in the header *and* the footer).
  - **Why the header copies were dropped.** § 18 Abs. 1 MStV requires the Impressum to be *leicht erkennbar*, *unmittelbar erreichbar* and *ständig verfügbar*; it does not require it to be visible on every screen. BGH, I ZR 228/03 (20.07.2006), held that an Impressum reachable through two links meets both the first two criteria, and read *unmittelbar erreichbar* as reachable without essential intermediate steps — a footer link is one click from every page. A permanent, clearly perceptible footer link is the recognised norm; the case-law risk (OLG Frankfurt) is a link set in very small or pale type, not its position.
  - **What that makes load-bearing, measured on production 2026-09-21:** footer links use `--accent` at `0.9rem` (14.4 px) — contrast **6.7:1** in light mode (`#1d4ed8` on `#ffffff`) and **10.4:1** in dark (`#93c5fd` on `#0b1120`), on every page, labelled exactly "Impressum". `tests/unit/test_site_build.py` pins those values, so a restyle that shrinks or fades the link fails CI rather than passing silently. **Verified on production after deploy `35586022540`:** the Impressum link is footer-only.
  - **The reasoning this replaces, kept because it is the residual risk.** A regulator warns against links visible only "erst nach langem Scrollen", and format pages are long: on them the footer link sits **below the fold**. That is accepted on the grounds above — the standard is reachability and perceptibility, not position — but it is the argument to revisit if the guidance or the case law moves.
  - **Rejected alternative:** one "Legal" link in the header to `/legal`, keeping the Impressum within two clicks from the top of every page. It would restore above-the-fold reachability at the cost of an extra step; one location was judged enough. The same applies to the Privacy link.
...
4. Write to `build/site/`: `search-index.json`, `favicon.ico` and `apple-touch-icon.png` (rendered from the brand mark; referenced by `<link rel="icon">` / `<link rel="apple-touch-icon">` on every page), `index.html`, `{format}`, `docs`, `docs/{slug}`, `legal`, `legal/{slug}`, `changelog`, `formats`, `status`, `assets/site.<hash>.css`, `assets/site.<hash>.js`, `assets/og.png`, `assets/mark.svg`, `manifest.json` (copied from the repository root), `formats.json`, `sha256sums.txt`, `llms.txt`, `llms-full.txt`, `sitemap.xml`, `robots.txt`, `.well-known/security.txt`, `schema/manifest-v1.json`, `_formats/{format}.json`, served at `/{format}/index.json` (ADR-032; site build is the single writer of these per-format files). Pages are written to disk as `{key}.html`, because `docs` and `docs/faq` cannot both be files.
5. Post-build checks (`tests/site/`, see `12` §3): every internal link resolves to a built key or a manifest path; no `http://` or external hosts except the allow-list (`github.com/kumarprabhashanand/loremfile`, `creativecommons.org`, `llmstxt.org`, `developers.cloudflare.com`); every page has exactly one H1, a `<title>`, a meta description, `lang="en"`; HTML validates with `html5lib` parse without errors; `assets/*.css` referenced with the right hash.
