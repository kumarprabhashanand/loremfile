...
**`loremfile_legal_pages_ai_agents` also matches Cloudflare's verified AI bot categories** (added 2026-09-22): `cf.verified_bot_category in {"AI Crawler" "AI Assistant" "AI Search" "Archiver"}`, joined to the token list with `or`. The four names are Cloudflare's own (verified bot categories, read 2026-09-22), and they catch a verified bot whose token nobody has added yet. `Archiver` — "Saves snapshots of websites to preserve digital content for historical records" — is in because an archived copy of these two pages would outlive any later fix here. `Search Engine Crawler` is a separate category and stays out: Googlebot, Bingbot and Applebot must reach the two pages to see `X-Robots-Tag: noindex`. **[VERIFY] resolved 2026-09-22, deploy run `35776508156`: the Free plan accepts `cf.verified_bot_category` in a custom rule.** Cloudflare's field reference states neither a plan restriction nor availability, so the first `infra apply` decided, exactly as `http.user_agent` did (ADR-030): `updated=1, failed=0` for `http_request_firewall_custom:loremfile_legal_pages_ai_agents`. The same run's `verify-live --mode smoke` then answered `290 checked, 0 failing`, which includes the four checks that ask for `403` on both legal pages as CCBot and PerplexityBot (`12` §4) — so the rule was in force at the edge within the same job, with no settle window needed.
