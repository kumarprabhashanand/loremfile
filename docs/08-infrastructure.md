...
{ "rules": [
  ...
  { "ref": "site_pages_headers", "description": "site pages (extensionless keys and index.html) and the API catalog", "enabled": true,
    "expression": "((not http.request.uri.path contains \".\") or ends_with(http.request.uri.path, \"/index.html\") or http.request.uri.path eq \"/.well-known/api-catalog\")",
    "action": "rewrite", "action_parameters": { "headers": {
      ...
      "Link": { "operation": "set", "value": "</llms.txt>; rel=\"describedby\", </.well-known/api-catalog>; rel=\"api-catalog\"" } } } },
...
**`files_noindex`** (split out of `files_headers` 2026-09-21). `X-Robots-Tag` used to sit in `files_headers`, so it reached every dotted path — including the four files that exist to be *shown*: `favicon.ico`, `apple-touch-icon.png`, `/assets/og.png`, `/assets/mark.svg`. Narrowing `files_headers` itself would also have stripped their `nosniff`, CORP and TAO, so the header moved to its own rule instead: `files_headers`' expression is **unchanged**, and `files_noindex` is that expression minus the four. Every fixture and data file therefore receives exactly the headers it did before. The exclusion uses `eq`, like `legal_pages_noindex`, rather than an unproven set literal, and the four paths are pinned against `routes.DISPLAY_ASSETS` by a test so the rule and the code that verifies it cannot drift apart. Both new rules never set a header another rule sets: `legal_pages_noindex` only matches extensionless paths. **This removes a wrong header; it is not the fix for the missing favicon in search.** Google's favicon documentation does not list `noindex` as a cause and the icon itself is valid (square ICO, 16/32/48 px), so that is most likely crawl latency, which Google describes as several days to several weeks. **Verified on production after deploy `35586022540` (2026-09-21):** the four display assets serve no `X-Robots-Tag` and do serve `nosniff`; fixtures still carry `noindex`.

**`loremfile_legal_pages_ai_agents` also matches Cloudflare's verified AI bot categories** (added 2026-09-22): `cf.verified_bot_category in {"AI Crawler" "AI Assistant" "AI Search"}`, joined to the token list with `or`. The three names are Cloudflare's own (verified bot categories, read 2026-09-22), and they catch a verified AI bot whose token nobody has added yet. `Search Engine Crawler` is a separate category and stays out: Googlebot, Bingbot and Applebot must reach the two pages to see `X-Robots-Tag: noindex`. **[VERIFY] whether the Free plan accepts `cf.verified_bot_category` in a custom rule.** Cloudflare's field reference states neither a plan restriction nor availability; the first `infra apply` after this change decides, exactly as `http.user_agent` did (ADR-030). If it is refused, the rule ships with the token list alone and this paragraph records that.
...
{ "rules": [
  { "ref": "loremfile_legal_pages_ai_agents", "description": "block self-identifying AI agents on the two legal pages that name the operator (ADR-028, ADR-030)", "enabled": true,
    "expression": "(http.request.uri.path eq \"/legal/imprint\" or http.request.uri.path eq \"/legal/imprint/\" or http.request.uri.path eq \"/legal/privacy\" or http.request.uri.path eq \"/legal/privacy/\") and (cf.verified_bot_category in {\"AI Crawler\" \"AI Assistant\" \"AI Search\"} or http.user_agent contains \"GPTBot\" or http.user_agent contains \"OAI-SearchBot\" or http.user_agent contains \"ChatGPT-User\" or http.user_agent contains \"OAI-AdsBot\" or http.user_agent contains \"ClaudeBot\" or http.user_agent contains \"Claude-User\" or http.user_agent contains \"Claude-SearchBot\" or http.user_agent contains \"CCBot\" or http.user_agent contains \"PerplexityBot\" or http.user_agent contains \"Perplexity-User\" or http.user_agent contains \"meta-externalagent\" or http.user_agent contains \"meta-externalfetcher\" or http.user_agent contains \"Amazonbot\" or http.user_agent contains \"Diffbot\" or http.user_agent contains \"MistralAI-User\" or http.user_agent contains \"DuckAssistBot\")",
    "action": "block" }
] }
```
...
- **The path scope, by hand.** At 07:11:42 UTC from colo TXL:
...
{ "records": [
  ...
  { "type": "TXT", "name": "loremfile.dev", "content": "google-site-verification=fAY7kvsrckmB6_mqBMw9QnpahH02wXnodltMh6I4M-Y" }
