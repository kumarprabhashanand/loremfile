...
| M5.4 | Hardening checklist `10` §5. **Evidenced:** DNSSEC; audit green; TLS/`always_use_https`; URL normalization; DMARC `p=reject`; bucket locks; the `main` ruleset; secret scanning, push protection, Dependabot, private vulnerability reporting; gitleaks. **Bucket locks re-verified by the owner 2026-09-16 against the Cloudflare API after M3.7–M3.9 added prefixes:** 80 rules, all enabled, all `Indefinite`, `edge/` present, `_locktest/` intact, `pdf/` unchanged. **To check:** Cloudflare hardware 2FA and recovery codes; token inventory; Registrar transfer lock and auto-renew; r2.dev disabled; no lifecycle rules; no other custom domains; SPF; `SECURITY.md`; `security.txt` | All boxes ticked |
| M5.5 | Tag `v1.1.0` — `catalog_version` is 1.1.0 and `release.check_tag` requires `tag == v<catalog_version>`, so the first release is v1.1.0, not v1.0.0; `release.yml` produces the full snapshot archive (first release: expect 223 members, 526,137,760 bytes); restore drill `11` §7.6 steps 1–3; then `infra.yml` `restore-dry-run` against the `v1.1.0` part (expect `skip=223`) | Archive verified; time recorded |
