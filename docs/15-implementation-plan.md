...
| ID | Task | DoD |
|---|---|---|
| M4.1 | Templates, CSS (light/dark tokens), minimal JS (copy, filter); `site/build.py`, `site/serve.py`; discovery files; JSON-LD. **Also**: the first extensionless key and the first `index.html` are the first objects that reach rule H3 at all, and the only ones that can exercise the `/index.html` **exclusion** in H1 and H2 — untested by staging because nothing it published could fire it (`09` §3.2). Verify all three branches against the published objects and record them there | `tests/site/*` green; Lighthouse (host command in `06` §9) a11y ≥ 95, informational; the three header branches verified |
