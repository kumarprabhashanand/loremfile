...
| M5.5 | Tag `v1.1.0`. **Released 2026-09-17:** run `35273804574` green, published 20:58:15, tag `694492a4aa` = `main`'s tip. Assets: `fixtures-snapshot-v1.1.0.part1.tar` (526,315,520 bytes), `manifest.json`, `sha256sums.txt`, `parts.txt`, `notes.md`. `catalog_version` is 1.1.0 and `release.check_tag` requires `tag == v<catalog_version>`, so the first release is v1.1.0, not v1.0.0. **Still to do:** the restore drill (`11` §7.6 steps 1–3) and `infra.yml` `restore-dry-run` against the `v1.1.0` part, expecting `skip=223` | Archive verified; time recorded |
