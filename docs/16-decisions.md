...
- **Context**: probe run 4 confirmed 404s are **not cached** — `edge_ttl.mode: respect_origin` with no `Cache-Control` from R2 on a 404 leaves nothing to respect (`03` §3, RISK-22). `19` §3 had assumed a 3-minute default. Cloudflare cache rules can cache by status code instead of relying on an origin header R2 never sends, so the obvious repair is a status-code TTL for 404.
