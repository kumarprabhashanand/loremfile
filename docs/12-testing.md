...
| Mode | Specified scope | Implemented | Measured at 228 fixtures (2026-09-22) |
|---|---|---|---|
| `smoke` | `/`, `/pdf`, `/pdf/`, `/manifest.json` count == local, one fixture per format (the smallest P1 fixture by bytes, ties by path order): HEAD status/length/type + header contract; OPTIONS preflight; Range on two files; `www` redirect | HEAD + the full header contract **by value**, one fixture per format (50); the live `/manifest.json` count and active entries against the checkout's; a CORS preflight for a ranged GET; `Range: bytes=0-99` on the smallest and the largest fixture over 100 bytes (the largest is above the multipart threshold); the `www` 301 keeping path and query, read without following it. With `--site-dir` (M4.1): `/`, a format page with and without its slash, and the legal pages | **84 s**, 288 checks, with `--site-dir` (home connection); **51 s**, 286 checks, in the deploy on GitHub runners. At 161 fixtures and without the site: 9 s, 50 checks |
| `daily` | smoke + HEAD every fixture (parallel, ≤ 20 rps) + GET/hash for all < 1 MB + 5 % rotating sample of larger + site-key hashes against the checkout rebuild + expiry checks (RDAP, TLS, security.txt) | HEAD + header contract on every fixture, GET/hash of every fixture < 1 MB (179 of 228), **sequential**. **Not implemented:** parallelism; the 5 % rotating sample — a fixture ≥ 1 MB is *never* hashed by `daily`. **Implemented 2026-09-15:** RDAP domain expiry (under 45 days), TLS certificate expiry (under 14 days), `security.txt` `Expires` (under 30 days), and with `--site-dir` every site key hashed against the rebuild plus the page header branches | **124 s**, 619 checks, with `--site-dir` (home connection). Not run on GitHub runners since `health.yml` moved to `full`; at 161 fixtures it took 21–99 s there. **No longer the scheduled check**: `health.yml` runs `full` since 2026-09-14, so the ≥ 1 MB gap in this row does not reach production monitoring |
| `full` | daily + GET/hash of **every** fixture (≈ 0.6 GB at launch, ≈ 1.1 GB after P1b) | HEAD + header contract + GET/hash of every fixture, sequential; the RDAP and TLS expiry checks and smoke's count, preflight, Range and `www` checks run here too; `daily`'s unimplemented checks do not | **192 s**, 668 checks, 595 MB, with `--site-dir` (home connection; the two legal pages differ there by design, because a local build keeps their placeholders). **116 s**, 666 checks, 0 failing, on GitHub runners (the scheduled health run; 154 s at 223 fixtures the day before). **The daily health check since 2026-09-14** |

**What the unimplemented checks will add, so the next measurement is not a surprise.** The expiry checks, and smoke's count, preflight, Range and redirect, are a handful of requests — seconds. Site-key hashes are dozens of small files — seconds. The one that moves the number is `daily`'s **rotating sample of fixtures ≥ 1 MB**: 49 of 228, up to 100 MB each, so a day that samples a large video can add a download measured in tens of megabytes. `daily`'s runtime is expected to grow mainly from that, not from catalog size.

**Re-measure the whole table, not one row,** when `daily`'s parallelism or rotating sample lands: the table was re-measured at 228 fixtures, after smoke's checks landed. Record all three columns each time. `health.yml` sizes its `verify-live` step timeout from the measured column, with the arithmetic in the workflow; when these numbers change, that timeout is checked against them in the same pull request. **The bound is 10 minutes for `full`**, plus 10 for the site-key retry: CI measured 116 s at 228 fixtures, about 5× under it. Revisit it if a CI run of `full` exceeds 5 minutes.
...
- [x] Open 10 fixtures in each browser of the matrix below (2026-09-17: PASS on all four — PDF inline, MP4 plays and seeks, MP3 plays, PNG/WebP/AVIF render, SVG renders, the embedded-PNG SVG shows its image, inline CSS styled; and `html/with-inline-js.html` executed nothing, Chrome logging the sandbox CSP refusal — **that log line is the rule working, not an error**) (PDF inline, MP4 plays with seeking, MP3 plays, PNG/WebP/AVIF render, SVG renders inert; `svg/with-embedded-png.svg` shows its embedded image; `html/with-inline-css.html` shows its styles; `html/with-inline-js.html` executes nothing).
- [x] `GET /html/basic%2Ehtml` carries the sandbox CSP (verified 2026-09-17).
- [x] (2026-09-17: PASS — the video played and the canvas drew untainted, "canvas OK") `<video src="https://loremfile.dev/mp4/720p-5s.mp4" crossorigin>` on a page at another origin plays; `fetch()` from another origin returns bytes; `<img crossorigin>` draws to canvas without taint. **Browser-only.** The headers underneath were verified 2026-09-17: a Range request returns 206 with `content-range`, `access-control-allow-origin: *`, `cross-origin-resource-policy: cross-origin` and `timing-allow-origin: *`; what remains is whether browsers behave as those headers promise.
- [ ] Upload-limit test: `10mib.bin` accepted and `10mib-plus-1.bin` rejected by a sample app with a 10 MiB limit.
- [x] `sha256sum -c` workflow from the docs works verbatim. **PASS (2026-09-17)** with `shasum -a 256 -c sha256sums.txt --ignore-missing`, 3/3 OK — which is why the docs and the site now give the macOS form beside the coreutils one.
- [x] Keyboard-only navigation of home and one format page; screen reader announces copy buttons; Lighthouse a11y ≥ 95. **PASS (2026-09-17):** keyboard navigation and VoiceOver announcements correct; **accessibility 100 on `/` and 100 on `/pdf/`**, against a bar of 95.
- [x] Mobile: no horizontal scroll on home/format/docs pages at 360 px width. **PASS (2026-09-17)** on both real devices, Android Chrome and iPhone Safari.
- [x] `curl -A ""`, `curl -A "python-requests/2.32"`, `curl -A "ClaudeBot/1.0"` and a plain `curl` all get 200 with no challenge (verified 2026-09-17).
- [x] REQ-27: run `health.yml` with `inject_failure=pdf/a4-3pages.pdf` → issue opened; run it again without → issue closed. **Drill 3, 2026-09-17** (`11` §7.11): run `35267683488` opened old issue #74 at 19:57:58, run `35268148973` closed it at 20:05:17, the two runs sequential.
...
**Fixture**: catalog entry with description/tags/expect/size class; generator + validator; deterministic test; documented in `05`; a CHANGELOG bullet (`09` §7); CI green; deployed; appears on the format page and in `llms-full.txt`.
