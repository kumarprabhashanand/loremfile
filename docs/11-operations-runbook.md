...
2. **Inside the pinned toolchain image** (only its output is authoritative; a host ffmpeg or Pillow will produce different bytes for media and images): `loremfile build --only <path> && loremfile validate --only <path> && loremfile manifest update` → commit `manifest.json` and `sha256sums.txt` changes (only additions). CI regenerates the fixture from the catalog and checks your committed entry against it; if it differs, the CI summary prints the exact entries to commit.
