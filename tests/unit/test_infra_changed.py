...
#: The cutoff as committed, captured at import — before any test patches it.
COMMITTED_CUTOFF = changed.HISTORY_REWRITTEN_AT


@pytest.fixture(autouse=True)
def _no_cutoff_unless_a_test_sets_one(monkeypatch: pytest.MonkeyPatch) -> None:
    """Every test here starts with no rewrite cutoff, whatever the repository has committed.

    The committed value is a fact about this repository's history, not about the rules under
    test. Without this, setting the cutoff made every older `decide()` test Undecided, because
    their runs carry no `created_at` — correct behaviour under a cutoff, wrong premise here.
    Tests that exercise the cutoff set one explicitly.
    """
    monkeypatch.setattr(changed, "HISTORY_REWRITTEN_AT", None)
...
REWRITE = dt.datetime(2026, 9, 21, 12, 0, tzinfo=dt.UTC)
BEFORE = "2026-09-20T10:00:00Z"
AFTER = "2026-09-21T13:00:00Z"


def test_without_a_rewrite_the_cutoff_changes_nothing() -> None:
    """Unset means no effect: `created_at` is not even read, so a run without one still counts."""
    base = select_base([run(5, "a")], exclude_run=CURRENT, rewritten_at=None)
    assert base == Base(run_id=1_005, sha="a" * 40)


def test_runs_from_before_a_rewrite_are_never_a_base() -> None:
    runs = [run(5, "a", created_at=BEFORE), run(6, "b", created_at=BEFORE)]
    assert select_base(runs, exclude_run=CURRENT, rewritten_at=REWRITE) is None


def test_the_latest_run_after_a_rewrite_is_the_base() -> None:
    runs = [run(5, "a", created_at=BEFORE), run(7, "b", created_at=AFTER)]
    assert select_base(runs, exclude_run=CURRENT, rewritten_at=REWRITE) == Base(1_007, "b" * 40)


def test_with_a_cutoff_a_run_without_a_usable_creation_time_is_undecided() -> None:
    """Never a guess: a run whose age cannot be read is neither kept nor dropped."""
    for created in (None, "not-a-date", "2026-09-21T13:00:00"):
        overrides = {} if created is None else {"created_at": created}
        with pytest.raises(Undecided, match="created_at"):
            select_base([run(7, "b", **overrides)], exclude_run=CURRENT, rewritten_at=REWRITE)


def test_the_first_push_deploy_after_a_rewrite_counts_infra_as_changed(
    monkeypatch: pytest.MonkeyPatch, history: dict[str, Any]
) -> None:
    """The case this exists for. The last successful deploy names a pre-rewrite SHA that this
    checkout does not have (`d` * 40 is in no repository)."""
    stale = [run(11, "d", created_at=BEFORE)]

    # Control: without the cutoff, the same run list fails the deploy, as it would after a
    # rewrite today.
    serve(monkeypatch, stale)
    with pytest.raises(Undecided, match="not a commit"):
        decide(
            repository="o/loremfile", exclude_run=CURRENT, head=history["head"], cwd=history["repo"]
        )

    monkeypatch.setattr(changed, "HISTORY_REWRITTEN_AT", REWRITE)
    decision = decide(
        repository="o/loremfile", exclude_run=CURRENT, head=history["head"], cwd=history["repo"]
    )
    assert decision.changed is True
    assert "history was rewritten at 2026-09-21T12:00:00+00:00" in decision.reason


def test_a_missing_base_after_the_rewrite_is_still_undecided(
    monkeypatch: pytest.MonkeyPatch, history: dict[str, Any]
) -> None:
    """The guard stays. A run from after the cutoff names a post-rewrite commit; if the
    checkout lacks it, something other than the rewrite is wrong, and deploy must stop."""
    monkeypatch.setattr(changed, "HISTORY_REWRITTEN_AT", REWRITE)
    serve(monkeypatch, [run(11, "d", created_at=AFTER)])
    with pytest.raises(Undecided, match="not a commit"):
        decide(
            repository="o/loremfile", exclude_run=CURRENT, head=history["head"], cwd=history["repo"]
        )
...
def test_a_cutoff_without_a_timezone_is_undecided() -> None:
    with pytest.raises(Undecided, match="timezone"):
        select_base([run(5, "a")], exclude_run=CURRENT, rewritten_at=dt.datetime(2026, 9, 21))


def test_the_committed_cutoff_is_unset_or_timezone_aware() -> None:
    """Whatever the constant is set to, it must be comparable with the API's UTC times."""
    cutoff = COMMITTED_CUTOFF
    assert cutoff is None or cutoff.tzinfo is not None
