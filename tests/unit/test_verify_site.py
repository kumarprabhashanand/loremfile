...
# --- display assets: production must serve them indexable ---------------------------------


def _asset(**headers: str) -> Response:
    return Response(status=200, headers={k.lower(): v for k, v in headers.items()})


def test_the_display_assets_are_the_four_the_rule_excludes() -> None:
    """Empty-set control: every check below is vacuous if this set were empty."""
    assert {
        "favicon.ico",
        "apple-touch-icon.png",
        "assets/og.png",
        "assets/mark.svg",
    } == routes.DISPLAY_ASSETS


def test_a_display_asset_without_noindex_and_with_nosniff_passes() -> None:
    responses = {
        key: _asset(**{"X-Content-Type-Options": "nosniff"}) for key in routes.DISPLAY_ASSETS
    }
    assert verify_live.display_asset_findings(responses) == []


def test_a_noindexed_display_asset_is_reported() -> None:
    """Negative control: the header this change removes must be caught if it comes back."""
    responses = {
        "favicon.ico": _asset(**{"X-Robots-Tag": "noindex", "X-Content-Type-Options": "nosniff"})
    }
    found = verify_live.display_asset_findings(responses)
    assert [(f.path, f.status) for f in found] == [("/favicon.ico", Status.HEADER_VALUE)]


def test_a_display_asset_that_lost_nosniff_is_reported() -> None:
    """The split must not cost the four their `nosniff`: that is the point of keeping
    `files_headers` whole rather than narrowing it."""
    found = verify_live.display_asset_findings({"assets/og.png": _asset()})
    assert [(f.path, f.status) for f in found] == [("/assets/og.png", Status.HEADER_MISSING)]
